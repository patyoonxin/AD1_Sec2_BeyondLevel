...
function ComplaintsPage() {
  ...
  const filteredComplaints = complaints.filter((c) => {
    ...
    const matchesSearch =
      !term ||
      [c.title, c.description, c.category, c.location, c.record_id].some(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMessage(
        `Complaint submitted successfully! Reference #${response.data.record_id || response.data.id}`,
      );
            ...
            <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-900">
                  {t("complaint_status", "Complaint Status")}
                </h2>
                <button
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="border-b pb-4">
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`badge ${getStatusBadge(selectedComplaint.status).badge}`}
                    >
                      {getStatusBadge(selectedComplaint.status).label}
                    </span>
                    <span className="text-sm text-gray-500">
                      {t("record_id", "Record ID")}:{" "}
                      {selectedComplaint.record_id || selectedComplaint.id}
                    </span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  {Array.isArray(selectedComplaint.responses) &&
                    selectedComplaint.responses.length > 0 && (
                      <div className="mt-4">
                        <p className="text-sm text-gray-600 mb-3 font-semibold">
                          {t("admin_responses", "Admin Responses")} (
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              {t("manage_complaints", "Manage Complaints")}
            </h1>
            <p className="text-gray-600 mt-2">
              {t(
                "page_subtitle_complaints",
                "Submit, monitor and manage all your complaints here",
              )}
            </p>
          </div>
          <button
        ...
        {showForm && (
          <div className="card mb-8 fade-in">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">
              {t("submit_new_complaint", "Submit New Complaint")}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {showEmptyBanner && (
                <div className="p-4 bg-red-50 border border-red-300 rounded-lg text-red-700 text-sm font-medium fade-in">
                  {t(
                    "all_fields_empty",
                    "Please enter your complaint details before submitting.",
                  )}
                </div>
              )}
              {/* Title */}
              <div className="form-group">
                ...
                <input
                  ...
                  style={
                    fieldErrors.title ? { borderColor: "#ef4444" } : undefined
                  }
                />
                {fieldErrors.title && (
              ...
              <div className="form-group">
                ...
                {aiSuggestion && !isAnalyzing && (
                  ...
                  >
                    ...
                    <span className="font-semibold">{aiSuggestion}</span>
                  </p>
                )}
                {aiError && !isAnalyzing && (
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder={t(
                    ...
                    "Enter the location (e.g., Jalan Merdeka, Kulai)",
                  )}
                  style={
                ...
                >
                  ...
                  <label htmlFor="file-input" className="cursor-pointer block">
                    <div className="flex justify-center">
                      <ArrowUpTrayIcon className="h-5 w-5 mr-2 text-3xl mb-2" />
                    </div>
                    <p className="text-sm font-medium text-gray-700">
                      {t("click_to_upload", "Click to upload or drag and drop")}
                    </p>
                    <p className="text-xs text-gray-500 mt-1">
                      {t("attachment_hint", "PDF, JPG, PNG, DOCX (Max 5MB)")}
                    </p>
                    {formData.attachment && (
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-5">
                ...
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder={t(
                      "placeholder_search_user_complaint",
                      "Search by title, description, category, location, or record ID...",
                    )}
            ...
            {complaints.length === 0 ? (
              <div className="empty-state">
                ...
                <p className="text-gray-500 text-sm mt-2">
                  {t(
                    "no_complaints_hint",
                    "Submit a complaint now to get help",
                  )}
                </p>
              </div>
              ...
              <div className="empty-state">
                <p className="empty-state-text">
                  {t("no_search_results", "No results match your search")}
                </p>
                <p className="text-gray-500 text-sm mt-2">
                  {t(
                ...
                {filteredComplaints.map((complaint) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div
                        className={`badge ${getStatusBadge(complaint.status).badge}`}
                      >
